...
class Player:
    ...
    def betRequest(self, game_state):
        ...
        try:
            ...
            if self.checkFigures(game_state) or self.checkLowerPairs(game_state):
                if int(game_state['current_buy_in']) - int(game_state['players'][playerIndex]['bet']) <= 300:
                    return int(game_state['current_buy_in']) - int(game_state['players'][playerIndex]['bet'])
            return 0
        except:
            traceback.print_exc()
            return 999999
    ...
    def checkLowerPairs(self, game_state):
        ...
        for player in playersList:
            if player['name'] == "Wolne Pythony":
                if str(player['hole_cards'][0]['rank']) == str(player['hole_cards'][1]['rank']) and str(player['hole_cards'][0]['rank']) in ["2","3","4","5","6"]:
                    return True
    ...
    def checkHigherPairs(self, game_state):
        ...
        for player in playersList:
            if player['name'] == "Wolne Pythony":
                if str(player['hole_cards'][0]['rank']) == str(player['hole_cards'][1]['rank']) and str(player['hole_cards'][0]['rank']) not in ["2","3","4","5","6"]:
                    return True
